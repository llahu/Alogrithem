...
public class DoublingTest {
    ...
    public static double timeTrial(int N) {
    	//为处理N个随机的6位整数的ThreeSum.count()进行计时
        int[] a = new int[N];//定义指定大小的整型数组
        for (int i = 0; i < N; i++) {
            a[i] = StdRandom.uniform(-MAXIMUM_INTEGER, MAXIMUM_INTEGER);//生成随机数
        }
    ...
    public static void main(String[] args) { 
        for (int N = 250; true; N += N) {//这将是一个死循环，知道N值超出MAX_INTz为止
