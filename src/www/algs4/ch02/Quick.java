...
public class Quick {
    ...
    private static void sort(Comparable[] a, int lo, int hi) { 
        if (hi <= lo) return;
        int j = partition(a, lo, hi);//确定切分元素的位置
        sort(a, lo, j-1);//将左子数组进行排序
        sort(a, j+1, hi);//将右子数组进行排序
    ...
    private static int partition(Comparable[] a, int lo, int hi) {
        ...
        while (true) { 
            ...
            while (less(a[++i], v))//找到i指针左边比切分元素大的元素
                if (i == hi) break;

            // find item on hi to swap
            while (less(v, a[--j]))//找到j指针右边比切分元素小的元素
